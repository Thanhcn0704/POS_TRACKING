...
- **EVENT-DRIVEN VACUUM = #1 Option A** (committed): replaced the dead-reckoning `threading.Timer` vacuum-ON with a closed-loop handshake. SCOL `DO_PICK` PRINTs `AT_PICK` at pick Z (after `WAIT MOTION>=100`) → `robot_link` `on_pick` → `send_relay(True)`; `REL` at discharge → `on_release` → OFF (+ belt-and-suspenders OFF after DONE). Mirrors the existing REL handler; removes the open-loop/GIL early-late-grip risk (B9 resolved). `LATENCY_OFFSET` no longer used for vacuum. **NEEDS THL400 reload** of `PICKTEST.scol`.
- **TCP RECONNECTION = #2 Phase 1** (THIS commit): `RobotLink` now owns the endpoint (`ip`/`port`) with `connect()`/`reconnect()` (capped exponential backoff, `stop_event`-interruptible). `_transact` tracks a `committed` flag: a **pre-commit** socket drop → `"reconnect"` → `send_verified` re-dials + retries the same object; **post-commit** drop → `"failed"` (robot moving, never double-sent). `main.py` lets the link own the socket. Fixes B5/B6 (zero-freeze holes). Phase 2 (split `thread_sender`) deferred. Hardware caveat: confirm a fresh Pi connect releases a blocked SCOL `INPUT` on the THL400.
- **SQUARE 40mm→50mm SIZE-GATE RETUNE** (THIS commit): the square part grew to a 50 mm side, which a 40 mm-tuned pipeline silently dropped. Measured the real ROI scale from `models/homography_test.npz` (~4.4 px/mm) → a 50 mm square is **2500 mm² / ~48k px²** (was 1600 mm² / ~31k px²), exceeding **two** maxima: pixel `AREA_MAX` (40000, used by `tools/collect_shapes.py` AND `detect_objects`) and `PHYSICAL_AREA_MAX` (2000 mm²). Raised in `config.py` (SSOT): `AREA_MAX` 40000→**65000**, `PHYSICAL_AREA_MAX` 2000→**3200**, `PHYSICAL_DIM_MAX` 65→**80** (restore the 40 mm part's proportional margin; 50 mm cleared 65 but headroom shrank). MINs untouched (parts only grew). Verified against the homography: both 40 mm and 50 mm squares now PASS all gates; all 111 tests green. Per the empirical guardrail the new bounds come from the *measured* homography scale, not a guess.
- **STM32 MOTOR-CONTROL REMOVED** (prev commit): deleted all conveyor-motor drive code/config (the in-progress BTS7960 migration, ex-L298N) from the firmware — `Motor_Run`/`Motor_Stop`, the `Motor_Run()` startup call, the PE11/PE12 GPIO init, the `GPIOEEN` clock enable, and the `MOTOR_*` defines + §4 of `config.h` (sections renumbered). **Encoder reading is fully retained** (TIM2 PA0/PA1, `ENCODER_PPR`, 100 ms telemetry); relays/UART/heartbeat untouched. The belt is now turned by an **external drive** (not the STM32); the firmware only *measures* it via ENC1. `HARDWARE_PINOUT.md` re-synced (PE11/PE12 freed; M1 = external). No Pi/Python change — `get_motor_snapshot()`/`get_motor_data()` are encoder telemetry, not motor control.
