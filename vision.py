...
import numpy as np
import config
from utils import pixel_to_robot
...
def classify_shape(contour):
    ...
    if circularity >= config.SHAPE_CIRCULARITY_CIRCLE:
    ...
    if vertices == 4:
        ...
        if aspect >= config.SHAPE_ASPECT_SQUARE_MIN:
            return "square", circularity, vertices
        if aspect <= config.SHAPE_ASPECT_RECT_MAX:
...
def run_detection(frame, roi_mask, hsv_params,
    ...
    for c in contours:
        a = cv2.contourArea(c)
        if not (config.AREA_MIN < a < config.AREA_MAX):
            continue
        hull_area = cv2.contourArea(cv2.convexHull(c))
        if hull_area == 0:
            continue
        sol = a / hull_area
        if sol < config.SOLIDITY_MIN:
            continue
        ...
        if not (config.PHYSICAL_AREA_MIN < phys_area < config.PHYSICAL_AREA_MAX):
            continue

        phys_rect = cv2.minAreaRect(c_phys)
        w_mm, h_mm = phys_rect[1]
        max_dim = max(w_mm, h_mm)
        if not (config.PHYSICAL_DIM_MIN < max_dim < config.PHYSICAL_DIM_MAX):
            continue
    ...
    return {
        ...
        "shape_code":  config.SHAPE_CODE.get(shape_name, config.SHAPE_CODE_DEFAULT),
    }, mask