...
def test_pick():
    # x_current=-100, v_belt=2000 -> t_obj=0.05 <= t_rob(0.1)+LAT(0.05)
    d = traj.evaluate(_entry(), -100.0, 2000.0, _LAST, _Pred(0.1), 28.0)
    assert d.action == traj.PICK, d


def test_wait():
    # t_obj = 5000/100 = 50s -> far -> WAIT
    d = traj.evaluate(_entry(), -5000.0, 100.0, _LAST, _Pred(0.1), 28.0)
    assert d.action == traj.WAIT, d


def test_hold():
    # t_obj = 25/100 = 0.25 -> inside (t_rob+LAT, t_rob+LAT+0.2] = (0.15, 0.35]
    d = traj.evaluate(_entry(), -25.0, 100.0, _LAST, _Pred(0.1), 28.0)
    assert d.action == traj.HOLD, d


def test_discard_passed():
    # x_current = 50 > X_OPT(0) -> already passed the fixed pick point
    d = traj.evaluate(_entry(), 50.0, 100.0, _LAST, _Pred(0.1), 28.0)
    assert d.action == traj.DISCARD, d


def test_reject_y_out_of_range():
    d = traj.evaluate(_entry(y=0.0), -100.0, 2000.0, _LAST, _Pred(0.1), 28.0)
...
def test_pick_targets_static_x_opt():
    ...
    assert abs(d.x_current - (-100.0)) < 1e-9
